function [metrics metricsInfo additionalInfo]=CLEAR_MOT_HUN_draw(gtInfo,stateInfo,options)
...
det_path = fullfile(options.dataDir, options.seqName, 'det', 'det.txt');
detections = dlmread(det_path);
track_path = fullfile(options.resDir, [options.seqName '.txt']);
tracks = dlmread(track_path);
ids = unique(tracks(:,2));
cmap = colormap;

file_video = [options.resDir, sprintf('videos/%s.avi', options.seqName)];
...
%     allfalsepos(t,1:length(falsepositives))=falsepositives;
    ...
    %% added by zhuji
    ax1 = subplot('Position', [0, 0.5, 0.5, 0.46]);
    img_path = fullfile(options.dataDir, options.seqName, 'img1', sprintf('%06d.jpg', t));
    img = imread(img_path);
    cla(ax1);
    imshow(img);
    title('Detections')
    hold on;
    text(26, 26, ['frame ' num2str(t)], 'BackgroundColor', [.7 .9 .7], 'FontSize', 16); 
    current_ind = find(detections(:,1) == t);
    for i = current_ind'
        x = detections(i, 3);
        y = detections(i, 4);
        w = detections(i, 5);
        ...
        r = detections(i, 7);
        str = sprintf('%.2f', r);
        rectangle('Position', [x y w h], 'EdgeColor', 'g', 'LineWidth', 2, 'LineStyle', '-');
        text(x, y-size(img,1)*0.01, str, 'BackgroundColor', [.7, .9, .7], 'FontSize', 8);
    end
    ax2 = subplot('Position', [0.5, 0.5, 0.5, 0.46]);
    cla(ax2);
    imshow(img);
    ...
    text(26, 26, ['frame ' num2str(t)], 'BackgroundColor', [.7 .9 .7], 'FontSize', 16); 
    current_ind = find(detections(:,1) == t);
    current_det = detections(current_ind, :);
    boxes = [current_det(:, 3), current_det(:, 4), current_det(:, 3) + current_det(:, 5), current_det(:, 4) + current_det(:, 6), current_det(:, 7)];
    index_nms = nms_new(boxes, 0.6);
    for i = index_nms'
        x = current_det(i, 3);
        y = current_det(i, 4);
        w = current_det(i, 5);
    ...
    ax3 = subplot('Position', [0.5, 0, 0.5, 0.46]);
    cla(ax3);
    imshow(img);
    title('Results');
    hold on;
    text(26, 26, ['frame ' num2str(t)], 'BackgroundColor', [.7 .9 .7], 'FontSize', 16); 
    current_ind = find(tracks(:,1) == t);
    current_tracks = tracks(current_ind, :);
    ids = unique(current_tracks(:,2));
    for i = current_ind'
        x = tracks(i, 3);
        y = tracks(i, 4);
        w = tracks(i, 5);
        ...
        c = cmap(index_color, :);
        rectangle('Position', [x y w h], 'EdgeColor', c, 'LineWidth', 2, 'LineStyle', '-');
        text(x, y-size(img,1)*0.01, str, 'BackgroundColor', [.7, .9, .7], 'FontSize', 8);
    end
    ax4 = subplot('Position', [0, 0, 0.5, 0.46]);
    cla(ax4);
    imshow(img);
    title('FP,FN,IDSW')
    hold on;
    ...
    for i = falsepositives
        x = stateInfo.Xi(t,i)-stateInfo.W(t,i)/2;
        y = stateInfo.Yi(t,i)-stateInfo.H(t,i);
        w = stateInfo.W(t,i);
        h = stateInfo.H(t,i);
        rectangle('Position', [x y w h], 'EdgeColor', 'r', 'LineWidth', 2, 'LineStyle', '-');
    end
    curgt = find(gtInd(t,:));
    curnottracked = setdiff(curgt, curtracked);
    for i = curnottracked
        x = gtInfo.Xi(t,i)-gtInfo.W(t,i)/2;
        y = gtInfo.Yi(t,i)-gtInfo.H(t,i);
        w = gtInfo.W(t,i);
        h = gtInfo.H(t,i);
        rectangle('Position', [x y w h], 'EdgeColor', 'b', 'LineWidth', 2, 'LineStyle', '-');
    end
    if t>1
        for ct=curtracked
            lastnotempty=find(M(1:t-1,ct),1,'last');
